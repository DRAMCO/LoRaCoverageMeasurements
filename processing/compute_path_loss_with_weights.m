...
function [PL0_ols, n_ols, sigma_ols, PL0_ml, n_ml, sigma_ml, PL0_est, n_est, sigma_est] = compute_path_loss_with_weights(file)
    ...
    %Maximum likelihood (ML) estimates
    pars = censoredml_with_weights(d,yt,c,t,PL0_ols,n_ols,sigma_ols,'linear')
    PL0_est= pars(1);
    n_est = pars(2);
    sigma_est = pars(3);
    ...
    pars = censoredml_with_weights(d,yt,c,t,PL0_ols,n_ols,sigma_ols, 'log')
    PL0_est= pars(1);
    n_est = pars(2);
    sigma_est = pars(3);
    ...
    pars = censoredml_with_weights(d,yt,c,t,PL0_ols,n_ols,sigma_ols, 'square')
    PL0_est= pars(1);
    n_est = pars(2);
    sigma_est = pars(3);
