...
with open(os.path.join(path_to_measurements, "measurements.json")) as f:
    ...
    for measurement in measurements:
        ...
        mat_file = F'censored_data_{measurement}_with_var_sigma.mat'
        sio.savemat(mat_file, {'y': y, 't': t_matlab, 'c': c, 'x': x, 'd' :d})
        (PLd0_ols, n_ols, sigma_ols, PLd0_ml, n_ml, sigma_ml, PLd0_est, n_est, a_est,
         b_est) = eng.compute_path_loss_with_var_sigma(mat_file,
                                                       nargout=10)
        ...
        sigma_est = a_est * np.log10(d) + b_est
        ax.plot(d, pl_ml, ls='-', label=r"MLS: \^\mu(d), \^\sigma(d)",
                linewidth=1.5, color='k')

        ax.plot(d, pl_ml + 2 * sigma_est, ls='--', linewidth=1, color='k')
        ax.plot(d, pl_ml - 2 * sigma_est, ls='--', linewidth=1, color='k')
        ...
        lm.format_axes(ax)
        lm.legend(plt)
        lm.save(plt, F'censored_data_{measurement}_with_var_sigma_path_loss.pdf')
